...
//         &&(bDiagPanel_AutoStartEnabled_g())
    ...
    case led_test:
      ...
      iUiCheckLed_SetState_g(eRunning);
      ...
      sprintf(caImagePath,"%s\\%s\\%s",
              ...
              WORK_ORDER_LED);
      if(!bUiCheckLed_SetWorkOrder_g(caImagePath))
      {
        ...
        if(!bUiCheckLed_SetWorkOrder_g(caImagePath))
        {
          printf("Datei für LED-Test nicht gefunden: %s\n",caImagePath);
        }
      }
      vUiSetUserText_g("prüfe LED");
      eState=led_test_init;
      break;
    ...
    case led_test_init:
      ...
      break;
    case led_test_set:
      eState=led_test_wait;
      break;
    case led_test_wait:
      // LED blinken bestätigen lassen
#ifdef SIMULATE_BOX
      if(bBtnIo)
      {
        bBtnIo=FALSE;
#else // !SIMULATE_BOX
      if(iReadDI_g(&eTasteIo))
      {
#endif // !SIMULATE_BOX
        eState=led_test_reset;
      }
      ...
      if(bBtnNio)
      {
        bBtnNio=FALSE;
#else // !SIMULATE_BOX
      if(iReadDI_g(&eTasteNio))
      {
#endif // !SIMULATE_BOX
        iUiCheckLed_SetState_g(eFail);
        tagAppStatus_g.iErrorCode=ERR_ZG_POS_2;
        vUiSetErrorText_g(tagAppStatus_g.iErrorCode,eState,"mind. eine Zeigerposition fehlerhaft");
    ...
    case ai_test:
      ...
      iUiCheckAdc_SetState_g(eRunning);
      vUiSetUserText_g("prüfe analoge Inputs");
      eState=ai_test_init;
      break;

    case ai_test_init:
      iHwTestIndex=DEV_AI_ADC;
      // TODO: ggf. entsprechende analoge Stimulation einschalten
      vDutCom_AnalogInputsInit();
      eState=ai_test_init_wait;
      break;
    case ai_test_init_wait:
    ...
    case ai_test_set:
      eState=ai_test_req;
      break;
    case ai_test_set_wait:
      if(dStatusTime>0.5)
        eState=ai_test_req;
      break;
    case ai_test_req:
      vDutCom_AnalogInputsRequestSingle(iHwTestIndex);
      eState=ai_test_read;
      break;
    case ai_test_read:
      if(dStatusTime>2.0)
      {
        // timeout
        tagAppStatus_g.iErrorCode=ERR_KOM_TIMEOUT;
        vUiSetErrorText_g(tagAppStatus_g.iErrorCode,eState,"Timeout Read analog Inputs");
        iUiCheckAdc_SetState_g(eFail);
        eState=process_end; //ignition_reset;
      }
      if(!bDutCom_AnalogInputsReadSingle(&uiAnalogInputValue))
        break;

      tagCurrentDeviceTyp_g.fUbatt=12.0f;   // Spannung ist von Netzteil fix auf 12V vorgegeben
      switch(iHwTestIndex)
      {
        case DEV_AI_ADC:
          uiAnalogInputValue=uiAnalogInputValue*10;
          iSoll=(int)(tagCurrentDeviceTyp_g.fUbatt*1000.0f);
          iMinValue=(int)((tagCurrentDeviceTyp_g.fUbatt-0.5f)*1000.0f);
          iMaxValue=(int)((tagCurrentDeviceTyp_g.fUbatt+0.5f)*1000.0f);
          pcUnit="mV";
          break;
      }
      switch(iHwTestIndex)
      {
        case 0: tagMesswerte_g.iUbatt =uiAnalogInputValue; break;
      }

#ifdef TEST_OFFLINE
      eState=ai_test_next;
#else // !TEST_OFFLINE
      if(  (uiAnalogInputValue<iMaxValue)
         &&(uiAnalogInputValue>iMinValue)
        )
      {
        // Analog Output within Limits
        eState=ai_test_next;
      }
      else
      {
        eErrorState=eState;
        tagAppStatus_g.iErrorCode=ERR_ANALOG_INPUT;
        vUiSetErrorText_g(tagAppStatus_g.iErrorCode,eState,
                          "Analog Input %s:\n"
                          "Soll=%d%s,\n"
                          "Ist=%d%s\n",
                          pcDutCom_AnalogInputsGetText(iHwTestIndex+1),
                          iSoll,pcUnit,
                          uiAnalogInputValue,pcUnit);
        iUiCheckAdc_SetState_g(eFail);
        eState=process_end;
      }
#endif // !TEST_OFFLINE
    ...
    case ai_test_next:
      ++iHwTestIndex;
      if(iHwTestIndex>DEV_AI_ADC)
      {
        iUiCheckAdc_SetState_g(ePass);
        eState=ai_test_end;
      }
      else
        eState=ai_test_set;
    ...
    case nfc_test:
      ...
      iUiCheckNfc_SetState_g(eRunning);
      vUiSetUserText_g("prüfe NFC");
      eState=nfc_test_init;
#else // !USE_NFC
      eState=nfc_test_end;
#endif // !USE_NFC
      break;
...
#ifdef USE_NFC
    case nfc_test_init:
      eState=nfc_test_init_wait;
      break;
    case nfc_test_init_wait:
      eState=nfc_test_set;
      break;

    case nfc_test_set:
      eState=nfc_test_set_wait;
      break;
    case nfc_test_set_wait:
      eState=nfc_test_req;
      break;

    case nfc_test_req:
      uiNfcCardCount=iNFC_ReadCardsOnReader_g();
      if(uiNfcCardCount!=1)
      {
        // kein NFC gelesen
        eErrorState=eState;
        tagAppStatus_g.iErrorCode=ERR_NFC;
        if(uiNfcCardCount<1)
          vUiSetErrorText_g(tagAppStatus_g.iErrorCode,eState,
                            "Kein NFC erkannt");
        else
          vUiSetErrorText_g(tagAppStatus_g.iErrorCode,eState,
                            "Mehrere NFCs erkannt");
        iUiCheckNfc_SetState_g(eFail);
        eState=process_end;
        break;
      }
      eState=nfc_test_read;
      break;
    case nfc_test_read:
      if(!pcNFC_GetUid_g(0))
      {
        ...
        tagAppStatus_g.iErrorCode=ERR_NFC;
        vUiSetErrorText_g(tagAppStatus_g.iErrorCode,eState,
                          "Fehler bei NFC Card lesen\n"
                          "\"%s\"",pcNFC_GetErrorMessage_g());
        iUiCheckNfc_SetState_g(eFail);
        eState=process_end;
        break;
      }
      strcpy(tagMesswerte_g.caNfcUid,pcNFC_GetUid_g(0));   // UID von NFC speichern
      eState=nfc_test_end;
      break;
#endif // !USE_NFC
